...
inline string _prefix_port_if_needed(const string &input) {
  static string const windows_com_port_prefix = "\\\\.\\";
  if (input.compare(0, windows_com_port_prefix.size(),
                    windows_com_port_prefix) != 0) {
...
void Serial::SerialImpl::open() {
  ...
  fd_ = CreateFileA(port_with_prefix.c_str(), GENERIC_READ | GENERIC_WRITE, 0,
